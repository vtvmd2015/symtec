...
class AjaxHelpers
{
	...
	static public function buttonActionCustom( $module  ,$method = 'newpage',$name,$icon)
	{
		$onclick = " onclick=\"ajaxViewDetail('#".$module."',this.href); return false; \"" ;
		if($method['form-method'] =='modal')
				$onclick = " onclick=\"SximoModal(this.href,'Create Detail'); return false; \"" ;
		...
		$html = '
			<a href="'.URL::to($module.'/update').'" class="tips btn btn-sm btn-primary"  title="'.$name.'" '.$onclick.'>
			<i class="fa fa-'.$icon.' "></i> '.$name.'</a>
		';
		return $html;
	}
